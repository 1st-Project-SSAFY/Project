    ...
    try:
        ...
        for i, direction in enumerate(directions):
            ...
            if current_direction != direction:
                ...
                setMotor(motor1, pwmA,  50, FORWARD)  # 방향 전환 전 속도 줄이기
                time.sleep(3)

                #방향 제어를 위한 후진
                setMotor(motor1, pwmA, 50, BACKWARD)  # 방향 전환 전 속도 줄이기
                time.sleep(2)
                setMotor(motor1, pwmA, 50, FORWARD)  # 방향 전환 전 속도 줄이기
                time.sleep(1)
                ...
                if next_direction != None:
                    if direction != next_direction:
                        setMotor(motor1, pwmA, 40, FORWARD)  # 전진 50% 속도    
                        time.sleep(1)
                    else:
                        setMotor(motor1, pwmA, 50, FORWARD)  # 전진 50% 속도
                        time.sleep(1.5)  # 서보와 동기화
                else:
                    setMotor(motor1, pwmA, 30, FORWARD)  # 전진 50% 속도    
                    time.sleep(0.5)  # 서보와 동기화
        ...
        setMotor(motor1, pwmA, 0, STOP)  # 모터 정지  # 모터 정지
