...
def control_motors(directions, path, stop_event):
    ...
    try:
        for current_direction_index, (direction, coord) in enumerate(zip(directions, path)):
            ...
            robot_data['info']['mission'] = 1

            # JSON 파일로 저장하기
            with open('/home/a204/qdrive/real_test/robot_data.json', 'w') as json_file:
            ...
            if current_direction != direction:
                ...
                if next_direction != None:
                    ...
                    if Drive_Mode ==1:
                        motor_controller.set_throttle(0.2*Drive_Mode)
                    else:
                        motor_controller.set_throttle(0.2*Drive_Mode)
...
def execute_turn(angle,Drive_Mode):
    ...
    if angle == 90 or angle == 150:
        ...
        motor_controller.set_throttle(0.8*Drive_Mode)
        time.sleep(1)
        motor_controller.set_throttle(0.6*Drive_Mode)
        time.sleep(1.5)
        if Drive_Mode == 1:
...
def go_drive(initial_direction, backward):
    ...
    try:
        ...
        with open('/home/a204/qdrive/real_test/directions.json', 'r') as file:
            directions_data = json.load(file)
            directions = directions_data.get('directions', [])

        with open('/home/a204/qdrive/real_test/path.json', 'r') as file:
        ...
        robot_data['info']['location_x'] =  19
        robot_data['info']['location_y'] =  17
        # JSON 파일로 저장하기
        with open('/home/a204/qdrive/real_test/robot_data.json', 'w') as json_file:
            json.dump(robot_data, json_file, indent=4)    
