...
from building_user.models import BuildingManager, BuildingManagerToken
from building_user.authentication import BuildingManagerTokenAuthentication
...
def signup(request):
    if request.method == 'GET':
        ...
        # 회원가입 시 입력한 소방관 고유번호
        input_pw = request.GET.get('number')
        ...
        if Firefighter.objects.filter(number=input_pw).exists():
    ...
    if request.method == 'POST':
        ...
        if serializer.is_valid(raise_exception=True):
            ...
            context = {
                'message':f"{input_name}님 가입이 완료되었습니다."
                }
...
import binascii
class CustomManagerAuthToken(APIView):
    def post(self, request, *args, **kwargs):
        username = request.data.get('username')
        password = request.data.get('password')
        user = authenticate(request, username=username, password=password)
        
        if user is not None and isinstance(user, BuildingManager):
            token, created = BuildingManagerToken.objects.get_or_create(user=user)
            context = {
                'key': token.key,
                'building_pk':user.building.pk
            }
            return Response(context, status=status.HTTP_200_OK)
        else:
...
def building_manager_logout(request):
    user = request.user  # 인증된 사용자
    try:
        # 해당 사용자의 토큰 삭제
        token = BuildingManagerToken.objects.get(user=user)
        token.delete()
